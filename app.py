...
if st.session_state.current_page == "Home":
    ...
    if time.time() - st.session_state.last_attempt_time < 10 and st.session_state.failed_attempts >= 3:
        ...
        if st.button("Login"):
            if login_pass == "arifa123":  # Hardcoded for demo, replace with proper auth
                reset_failed_attempts()
                st.success("✅ Reauthorized successfully!")
                st.session_state.current_page = "Home"
                st.rerun()  # Updated from experimental_rerun() yt 
            else:
                st.error("❌ Incorrect password!")

# Add a footer
st.markdown("---")
st.markdown("🔐 Secure Data Encryption System - AI Project")